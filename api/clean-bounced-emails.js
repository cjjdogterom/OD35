...
module.exports = async function handler(req, res) {
  ...
  if (!serviceKey) {
    return res.status(500).json({ error: 'SUPABASE_SERVICE_ROLE_KEY niet geconfigureerd' });
  }
  ...
  if (expectedSecret && secret === expectedSecret) {
    ...
    try {
      const r = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
        headers: {
          apikey: process.env.SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im14YnRtYmNneWNqcWFwanp1bHJwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc5MDQ4MzgsImV4cCI6MjA5MzQ4MDgzOH0.6Qowa_mqhH7YtrljF5fZnQzUaG_u4N5TodcnLzRhYSM',
      ...
      if (r.ok) {
        ...
        authorized = email && !email.startsWith('viewer@') && !email.includes('viewer');
      }
    } catch (_) {}
  }
  ...
  for (const [id, patch] of Object.entries(perPerson)) {
    ...
    if (updateError) {
      ...
      if (errors.length < 5) errors.push({ id, message: updateError.message });
    } else {
